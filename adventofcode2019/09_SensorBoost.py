...
class IntCodeProcessor:
    ...
    def process_instruction(self):
        ...
        # Relative Base Offset Instruction
            ...
            self.relative_cntr += self.fetch_arg(arg_refstr, 0)
            self.pcntr += 2
        # Halt Instruction
        elif instr == 99:
            self.finished = True
    ...
    def fetch_arg(self, arg_refstr, arg_index):
        ...
        # Immediate Mode
            ...
            address = self.pcntr + 1 + arg_index
        # Relative Mode
        elif arg_mode == 2:
            address = self.relative_cntr + self.data_tape[self.pcntr + 1 + arg_index]
        else:
            raise ValueError("Invalid ArgMode Passed!")

        if address >= len(self.data_tape):
            new_data = [0] * (address - len(self.data_tape) + 1)

            self.data_tape.extend(new_data)
    ...
    def put_value(self, arg_refstr, arg_index, value):
        ...
        # Relative Mode
            ...
            write_index = self.relative_cntr + self.data_tape[self.pcntr + 1 + arg_index]
        else:
            raise ValueError("Invalid ArgMode Passed!")

        if write_index >= len(self.data_tape):
            new_data = [0] * (write_index - len(self.data_tape) + 1)
...
input_file = 'adventofcode2019/09_SensorBoost.txt'
...
if __name__ == '__main__':
    ...
    processor = IntCodeProcessor(instr_arr, input_vals)
